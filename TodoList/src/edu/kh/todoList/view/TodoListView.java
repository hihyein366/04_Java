...
public class TodoListView {
	...
	private void todoDetailView() throws NumberFormatException, IOException {
		...
		if(todoDetail == null) {
			System.out.println("### 입력한 인덱스 번호가 존재하지 않다 ###");
			return;
		}
	...
	private void todoAdd() throws IOException {
		...
		if(index == -1) { // 실패
			System.out.println("### 추가 실패래요~~~ ###");
			return;
		}
		
		// 성공
		System.out.printf("[%d] 인덱스에 추가 되었다람쥐\n", index);
	...
	private void todoComplete() throws NumberFormatException, IOException {
		...
		if(result) { // 변경 성공 
			System.out.println("[변경 되어따리!~]");
			
		} else { // index 요소가 범위를 초과한 경우
			System.out.println("### 인덱스 번호 제대로 써라 ㅡ,.ㅡ ###");
		}
	...
	private void todoUpdate() throws NumberFormatException, IOException {
		...
		if(result) System.out.println("[수정이 되었어...!]");
		else System.out.println("### 안타깝지만 수정 실패입니다.. ###");
	...
	private void todoDelete() throws NumberFormatException, IOException {
		...
		if(title == null) System.out.println("### 인덱스 노존재 ###");
		else System.out.printf("[%s]가 삭제 되었습니다람쥐\n", title);
