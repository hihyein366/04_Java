...
public class ScannerEx1 {
	...
	public static void main(String[] args) {
		 ...
		 *  -> import 구문 작성 필요
		 * 
		 * - Scanner.next() : 단어 1개 입력. 공백 문자 입력 시 종료
		 * 
		 * - Scanner.nextLine() : 문자열 1개 입력. 엔터(개행문자) 입력 시 종료 
		 * 
		...
		int num1 = sc.nextInt(); // 다음 입력된 정수 얻어와 num1에 대입
		...
		//  nextLine() 작성해야 하는 상황이 오면 nextLine() 구문을 미리 한 번 작성하기
		...
		// -> 이때 정수와 같이 입력된 엔터가 버퍼에 남아있게 됨
		
		// 이후 nextLine() 해석 시 
		// 버퍼에서 다음 엔터까지 읽어오는 작업이 수행되는데 
		// 버퍼에 남아있던 엔터 읽어오기 때문에 아무것도 입력못함
		
		// [해결방법]
		// nextLine() 한번 호출해서 남아있던 엔터 빼버림(제거)
