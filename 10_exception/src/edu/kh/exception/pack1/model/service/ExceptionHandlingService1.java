...
public class ExceptionHandlingService1 {
	...
	public void method1() {
		...
		/* 예외 발생 원리 */
		// 실행 중 예외 상황이 발생하는 경우
		// 예외 상황에 맞는 코드가 수행되며 지정된 예외 클래스를 찾아 객체로 만들어서 던짐
	 ...
	 * 
	 * - try{} : 예외가 발생할 것 같은(가능성 있는) 코드를 시도하는 구문
	 * 
	 * - catch(예외){} : try 구문 시도 중 예외가 발생되어 던져진 경우
	 * 									 catch(예외)에 작성된 예외와 던져진 예외 같으면 잡아서 {} 내 구문 수행
	 * 									-> 발생된 예외 잡아서 처리했기 때문에 프로그램 멈추지 않고 정상수행 
	 */
	public void method2() {
		int[] arr = {10,20,30,40}; // 마지막 인덱스 == 3
	...
	public void method4() {
		...
		// 입력된 값이 다른 자료형인 경우 발생되는(던져지는) 예외
		
		System.out.println("[두 수를 입력 받아 나눈 몫 구하기]");
		
		int num1 = 0; // try 밖으로 뺌. cuz. try지역변수라 catch에서 접근 불가
		int num2 = 0;
		...
		try {
			...
			// 입력 관련 예외가 먼저+자주 발생할 가능성 -> 먼저 처리하는게 좋다
			System.out.println("입력이 올바르지 않자나!!!!!!!!!");
	 ...
	 * 1) throw : 예외 강제 발생(던짐)
	 * 2) 예외 + 다형성  
	 *  - catch문 여러 개 작성 시 부모 예외를 처리하는 catch문이 자식 예외도 모두 잡아서 처리
	 *   -> 부모 예외는 아래쪽에 작성해야 컴파일 에러 없음
	 */
	public void method5() {
		...
		do {
			try {
				...
				default : System.out.println("[메뉴에 작성 된 번호만 입력해라 아 아아!]");
				}
				
			} // try end
			
			// catch (RuntimeException e) 이게 제일 위에서 처리되면
			// 부모 타입 예외 참조 변수 = 자식 타입 예외 객체
			// -> 다형성 업캐스팅 (매개 변수) = new ArithmeticException()
			
			// 자식 예외를 부모 catch문에서 모두 잡아서 처리하기 때문에
			// 자식 catch문까지 코드 도달할수 없다. (수행불가, 죽은코드)
			
			// -> 해결 방법 : 부모 catch를 자식 catch 밑에 놓는다
				...
				// sc.nextInt() 구문이 계속 예외 발생!   ex) 3 f < 이렇게 치면 3 띄어쓰기 후 f 무한반복
				sc.nextLine();
...
//			throw new EOFException();
		...
		// 파이널 프로젝트, 실제 서비스 운영
		// -> 모든 예외 상황에 맞게 대비 (코드 길이 늘어나지만 UX 증가)
