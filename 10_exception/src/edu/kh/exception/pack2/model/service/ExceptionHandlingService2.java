...
public class ExceptionHandlingService2 {
	 ...
	 * 
	 * - try 구문에서 예외 발생 여부와 관계 없이 무조건 수행해야 되는 구문을 작성
	 * 
	...
	public void method1() {
		...
		try {
			...
			else throw new RuntimeException(); // 예외 객체 생성 후 던짐(throw) 던져야 캐치로 받는다
	...
	public void method2() {
		...
		// -> 프로그램 수행 후 scan이 메모리에 남아있어 메모리 누수(쓸데 없는 메모리 차지)발생
		
		// [해결 방법] Scanner를 close(메모리 반환) 하기
		// -> 코드에서 예외가 발생 하덩말덩 무조건 close 수행 필수~!
		//  --> finally가 이 코드를 수행하는 데 부합함
