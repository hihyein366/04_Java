...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		try {
			...
			sb.append("~!@#$%^&*()_+ \n");
			sb.append("가나다라마바사\n");
			sb.append("내가 누구? 백현 콘서트 가는 자");
			sb.append(" 내 손으로 잡은 나의 티켓");
			sb.append(" 그 것은 나의 것");
			sb.append(" 나의 노력");
			sb.append(" 나의 행운");
			...
			//		1byte 기반 스트림 통과 시 데이터 손실 발생해서 글자 깨지는 문제 발생
...
//			}
			...
			//		   							현재 시간까지 차이를 ns(nano second) 단위로 반환
			// 1ms(밀리) 		 == 1/1000초
			// 1us(마이크로) == 1/1000ms
	...
	public void bufferedFileByteOutput() {
		...
		/* 버퍼(Buffer)란?
		 * - 데이터를 모아두는 박스 같은 메모리 공간
		 * 
		 * * 버퍼링
		 * - 인터넷 속도 낮음 + 스트리밍 코드가 효율이 안좋음
		 *  -> 동영상 재생 중에 미리 앞에 재생될 부분을 다운 받아놔서 영상 끊김 없이 볼수 있게 함
		 *   --> 다운 받아서 저장 해놓은 공간 == 버퍼 
		 */ 
		
		/* BufferedOutputStream / BufferedInputStream
		 * 
		 * - FileOutputStream은 1byte씩 데이터를 입출력
		 * 
		 * - BufferedOutputStream / BufferedInputStream
		 *  -> 버퍼에 입출력할 내용을 모아서 한 번에 입출력!
		 *  
		 *  --> 스트림을 이용하는 횟수가 적어짐 -> 성능, 시간 효율 up! 
		 */
	...
	public void fileByteInput() {
		...
		try {
			fis = new FileInputStream("\\io_test\\20240215\\이한영.txt");
	...
	public void fileByteInput2() {
		...
		try {
			...
			//				-> 이 후 byte[] 이용해서 String 객체 생성
			//				-> String 생성 시 배열 요소 2개씩 묶어 한 글자로 해석해 깨지는 글자 없이 문자열로 변환
	...
	public void bufferedFileByteInput() {
		...
		} catch(Exception e) { // Exception은 IOException의 부모라 익셉션으로 처리해도 ok
			e.printStackTrace();
	 ...
	 * 
	 * [실행화면]
	 * 파일 경로 입력 : /io_test/20240215/이한영.txt
	 * 복사 완료 : 			/io_test/20240215/이한영_copy.txt
	 * 
	 * 1) 입력된 경로에 파일이 있는지 검사 : File.exists()
	 * 2) 있으면 파일 내용 모두 읽어오기 : FileInputStream + BufferedInputStream
	 * 3) 읽어온 내용 같은 위치에 "파일명_copy" 이름으로 출력 : FileOutputStream + BufferedOutputStream
	 * 
	...
	public void fileCopy() {
		...
		try {
			...
			if(!fi.exists()) { // 해당 경로에 파일이 존재하지 않으면
			...
			StringBuilder sb = new StringBuilder();
			sb.append(target); // /io_test/20240215/이한영.txt
			
			// int String.lastIndexOf("문자열")
			// - String 뒤에서부터 검색해서 "문자열"과 일치하는 부분의 인덱스 반환. 없으면 -1 반환
