...
public class Member {
	...
	//								-> 붙은 이유 : static 메모리에 할당될 때 클래스명.변수명으로 할당되기 때문
	// - 인스턴스 변수 : static이 없는 변수
	//								-> 없는 이유 : 인스턴스(객체)가 생성될 때 heap 메모리에 할당되기 때문
	...
	public Member() {
		...
		// 생성 시 객체 초기화
		memberId = "ID를 입력해 줘 제발";
		memberPw = "Pw를 입력해 줘 제발";
		memberName = "Name을 입력해 줘 제발";
	}
	
	
	
	/* 매개 변수 생성자 */
	// - 객체 생성 시 매개 변수로 전달된 값을 생성된 객체의 필드에 초기화 하는 용도의 생성자
	...
	// 오버 로딩 안되는 경우(위에 거랑 자료형 순서, 개수 겹침)
	// Duplicate method Member(int, String) in type Member
	// --> 오버 로딩은 변수명 아닌 자료형 개수, 순서, 타입 중 하나라도 달라야 성립.
	...
	// setter : 전달 받은 매개변수를 현재 객체의 필드에 대입
