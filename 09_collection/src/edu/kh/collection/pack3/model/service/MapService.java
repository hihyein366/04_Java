...
public class MapService {
	 ...
	 * 
	 * * Key만 묶어서 보면 Set와 같은 특징을 지니고 있음.
	 *  -> Map.keySet() / Map.EntrySet() 제공
	 *  
	 *  
	 * Value : 키워드에 해당하는 상세한 값
	 * - Key로 인해서 모든 Value가 구문됨
	...
	public void method1() {
		...
		// - Map에 추가 시 중복되는 Key 없으면 null, 있음 이전 value 반환
		
		System.out.println(map.put(1, "에그마요"));
		System.out.println(map.put(2, "로티세리"));
		...
		// 2. V get(K k) : Key에 해당되는 Value 반환. 일치 키 없음 null 반환
		...
		// 4. V remove(K k) :
		// - Key가 일치하는 Entry를 제거
		// - 일치하는 Key가 있다면 Value, 없음 null 반환
